	...
	<table class="table table-bordered datalist table-hover">
      <thead>
        <tr>
			...
			<th width=250>
				<?php if($action_add != false): ?><a href="<?php echo U($action_add['node_name'],array('act_type'=>1));?>" class="btn btn-primary btn-xs">
      ...
      <tbody>
      	<?php if(is_array($nav_menu)): foreach($nav_menu as $key=>$row): ?><tr style="background:#d9edf7">
				<td>
					<span class="d-toggle" data="toggle_<?php echo ($row['id']); ?>">
					 	<?php echo ($row['title']); ?>&nbsp;&nbsp;[展开/隐藏]
					 </span>
				</td>
				...
				<td>
					<?php if(is_array($action_lists)): $i = 0; $__LIST__ = $action_lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$act_row): $mod = ($i % 2 );++$i;?><a href="<?php echo U($act_row['node_name'],array('act_type'=>1,'id'=>$row['id']));?>"  onclick="<?php echo ($act_row['pre_func']); ?>();"><?php echo ($act_row['title']); ?></a>&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
			        <a href="<?php echo U($action_add['node_name'],array('act_type'=>2,'id'=>$row['id']));?>" class="btn btn-info btn-xs mybtn">
		          增加侧栏一级菜单
		        	</a>
				</td>
	        </tr>
	        <?php $controller_menu=get_node_auth(array('pid='.$row['id']));?>
	        <?php if($controller_menu != 0): if(is_array($controller_menu)): foreach($controller_menu as $key1=>$row1): ?><tr class="toggle_<?php echo ($row['id']); ?> to_hide">
						<td>
						 	<span style="font-family: fantasy;">├─&nbsp;</span><?php echo ($row1['title']); ?>
						</td>
						<td>
						 	<?php echo ($row1['node_name']); ?>
						</td>
						<td>
							<?php if(is_array($action_lists)): $i = 0; $__LIST__ = $action_lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$act_row): $mod = ($i % 2 );++$i;?><a href="<?php echo U($act_row['node_name'],array('act_type'=>2,'id'=>$row1['id']));?>"  onclick="<?php echo ($act_row['pre_func']); ?>();"><?php echo ($act_row['title']); ?></a>&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
					        <a href="<?php echo U($action_add['node_name'],array('act_type'=>3,'id'=>$row1['id']));?>" class="btn btn-info btn-xs mybtn">
		          增加侧栏二级菜单
		        	</a>
						</td>
			        </tr>
			        ...
			        <?php if($sec_menu != 0): if(is_array($sec_menu)): foreach($sec_menu as $key2=>$row2): ?><tr class="toggle_<?php echo ($row['id']); ?> to_hide">
								<td>
								 	<span style="font-family: fantasy;">│-----├─&nbsp;</span><?php echo ($row2['title']); ?>
								</td>

								<td>
								 	<?php echo ($row2['node_name']); ?>
								</td>


								<td>
									<?php if(is_array($action_lists)): $i = 0; $__LIST__ = $action_lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$act_row): $mod = ($i % 2 );++$i;?><a href="<?php echo U($act_row['node_name'],array('act_type'=>3,'id'=>$row2['id']));?>"  onclick="<?php echo ($act_row['pre_func']); ?>();"><?php echo ($act_row['title']); ?></a>&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
							        <a href="<?php echo U($action_add['node_name'],array('act_type'=>4,'id'=>$row2['id']));?>" class="btn btn-success btn-xs mybtn">
		         				 	增加操作
		        					</a>
								</td>
					        </tr>
					        <?php $action_menu=get_node_auth(array('pid='.$row2['id']));?>
					        <?php if($action_menu != 0): if(is_array($action_menu)): $key3 = 0; $__LIST__ = $action_menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row3): $mod = ($key3 % 2 );++$key3;?><tr class="toggle_<?php echo ($row['id']); ?> to_hide">
										<td style="color:#ABABAB">
										 	<span style="font-family: fantasy;">│-----│-----├─&nbsp;</span><?php echo ($row3['title']); ?>
										</td>
										<td>
										 	<?php echo ($row3['node_name']); ?>
										</td>
										<td>
											<?php if(is_array($action_lists)): $i = 0; $__LIST__ = $action_lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$act_row): $mod = ($i % 2 );++$i;?><a href="<?php echo U($act_row['node_name'],array('act_type'=>4,'id'=>$row3['id']));?>"   onclick="<?php echo ($act_row['pre_func']); ?>();"><?php echo ($act_row['title']); ?></a>&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
										</td>

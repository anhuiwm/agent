...
<body>
 ...
 <?php if(is_array($SEC_MENU)): foreach($SEC_MENU as $key=>$row): ?><div class="panel panel-default">
    ...
    <form method="post" class="myforms" action="<?php echo U('GmTool/Command/base');?>">
        <input type="text" class="form-control" placeholder="GameID" name="gameid"> 或
        <input type="text" class="form-control" value="<?php echo ($curuser); ?>" placeholder="UserID" name="userid">或
        <input type="text" class="form-control" placeholder="账户" name="accountname">或
    ...
    <table class="table  datalist  table-hover">
        ...
        <tbody>
            <?php if(is_array($list_data)): foreach($list_data as $key=>$row): ?><tr>
                    ...
                    <?php if(is_array($title_lists)): foreach($title_lists as $key1=>$row1): ?><th>
                            <?php echo ($key); ?>
                        </th>
                        <th>
                            <?php echo ($row); ?>
